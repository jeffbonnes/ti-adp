		...
		tiadpHelper.showADPLoginDetails(function(res) {
			if (res != null) {
				if (res.length > 0) {
				    var padding = "                                                   ";
				    console.log( "---------------------------------------------------------------------------------------------");
				    console.log( "     " + ("Team" + padding).substr(0,50) + " Login"  );
				    console.log( "---------------------------------------------------------------------------------------------");
					tiadpHelper.getTeamsFromArray(res, function(loginDetails) {
						for ( i = 0; i < loginDetails.length; i++) {
						    var num = (padding + (i + 1) + ". " ).substr( -5 );
							var teamName = (loginDetails[i].teamName + padding).substr(0, 50);
							console.log( num+ teamName + " " + loginDetails[i].login);
						}
					});
					console.log(" ");
				} else {
